...
require_relative File.join '..', 'lib', 'opushon'
require_relative 'helper'

subject 'OPTIONS /issues' do
  # HTTP/1.1 200 OK
...
it 'is expected to load the JSON' do
  Expect.this { Opushon.load(@object).to_h }.to eql: {
    :GET=>{
      :title=>"List issues",
      :description=>"List all issues across all the authenticated user's visible repositories.",
      :request=>{
...
it 'is expected to dump the JSON' do
  opushon = Opushon.load(@object)

  Expect.this { JSON.parse(Opushon.dump(opushon), symbolize_names: true) }.to eql: {
    :GET=>{
      :title=>"List issues",
      :description=>"List all issues across all the authenticated user's visible repositories.",
      :request=>{
